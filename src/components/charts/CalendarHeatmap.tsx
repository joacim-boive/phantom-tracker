...
export function CalendarHeatmap({ entries, isLoading }: CalendarHeatmapProps) {
  function calculateWeeksData() {
    ...
    const entriesByDate = new Map<
      string,
      { count: number; avgPain: number; maxPain: number }
    >();
    entries.forEach((entry) => {
      ...
      if (existing) {
        ...
        entriesByDate.set(dateKey, {
          count: newCount,
          avgPain:
            (existing.avgPain * existing.count + entry.pain_level) / newCount,
  ...
  const { weeks } = calculateWeeksData();
